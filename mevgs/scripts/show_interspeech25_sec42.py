...
def quantify_variance():
    ...
    def compute1(modality, words_type, seed, var_type):
        ...
        if var_type == "full":
            ...
            vars = [
                compute_var(select_by_word(data, embs, word))
                for word in WORDS[words_type]
            ]
            return np.mean(vars)
        else:
    ...
    results = [
        {
            ...
            "seed": seed,
            "value": compute1(modality, words_type, seed, var_type),
        }
    ...
    df = df.replace(
        {
            ...
            "var_type": {"full": "Full", "concept": "Within concept"},
        }
    )
